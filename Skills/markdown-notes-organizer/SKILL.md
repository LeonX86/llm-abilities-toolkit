...
使用引用块 + `[!类型]` 语法创建提示框。五种类型及对应颜色：`NOTE` 🔵蓝、`TIP` 🟢绿、`IMPORTANT` 🔵蓝、`WARNING` 🟡黄、`CAUTION` 🟡黄。

- 自定义标题：`> [!TIP] 标题内容`
- 可折叠：类型后加 `-` 折叠 / `+` 展开
- 嵌套：通过引用块嵌套实现，如 `> > [!NOTE] 内容`
- 一个文档中 callout 数量适中，避免视觉疲劳，不连续堆叠
- 保持简洁，callout 内容不宜过长
...
- 若原文带有口语化、对话式或第二人称表达，统一改写为陈述性文字，使笔记读起来像知识文档
- **禁止遗漏任何有效信息**
...
加粗（`**文字**`）只用于极少数真正关键的词语，全篇占比不超过 2%：

- 只加粗核心关键词，禁止对整句长句加粗（等同于没加粗，失去意义）
- 同一段落内加粗不超过 1～2 处
